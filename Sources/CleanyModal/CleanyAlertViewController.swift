...
open class CleanyAlertViewController: CleanyModalViewController {
    ...
    public var bottomOffset: CGFloat = 20.0
...
//        )
        ...
        self.dataSource = AlertModel(title: title, attributedTitle: nil, attributedMessage: nil, message: message, iconName: imageName)
        self.styleSettings = styleSettings ?? CleanyAlertConfig.getDefaultStyleSettings()
        self.preferredStyle = preferredStyle
        self.widthMultiplier = preferredStyle.widthMultiplier
        
        super.init(nibName: "CleanyAlertViewController", bundle: Bundle(for: CleanyAlertViewController.self))
    }
    
    public init(attributedTitle: NSAttributedString? = nil, attributedMessage: NSAttributedString? = nil, imageName: String? = nil, preferredStyle: CleanyAlertViewController.Style = .alert, styleSettings: CleanyAlertConfig.StyleSettings? = nil) {
        
//        precondition(
//            attributedTitle != nil || attributedMessage != nil, "NOPE ! Why you would like to show an alert without at least a title OR a message ?!"
//        )
        self.dataSource = AlertModel(title: nil, attributedTitle: attributedTitle, attributedMessage: attributedMessage, message: nil, iconName: imageName)
        self.styleSettings = styleSettings ?? CleanyAlertConfig.getDefaultStyleSettings()
        self.preferredStyle = preferredStyle
        self.widthMultiplier = preferredStyle.widthMultiplier
    ...
    open override func viewDidLoad() {
        ...
        } else if dataSource.message == nil && dataSource.attributedMessage == nil {
        ...
        case .actionSheet:
            ...
            alertViewBottom?.constant = bottomOffset
        ...
        var widthMultiplier = preferredStyle.widthMultiplier
        if let multiplier = self.widthMultiplier, multiplier >= preferredStyle.widthMultiplier , multiplier <= 1 {
            widthMultiplier = multiplier
        }
        alertWidthConstraint = alertWidthConstraint.setSafelyMultiplier(multiplier: widthMultiplier)
        //
        // ***** End style
        
        applyStyle()
        titleLB.text = dataSource.title
        if let attributedTitle = dataSource.attributedTitle {
            titleLB.attributedText = attributedTitle
        }
        messageLB.text = dataSource.message
        if let attributedMessage = dataSource.attributedMessage {
            messageLB.attributedText = attributedMessage
        }
        ...
        _stackedViews?.forEach({ viewTuple in
            ...
            if let height = viewTuple.height {
    ...
    open func addCustomViewInContentStack(_ view: UIView, height: CGFloat? = nil) {
    ...
    private func applyStyle() {
        ...
        if let mainViewMaskedBounds = styleSettings[.mainViewMaskedBounds] {
            if #available(iOS 11.0, *) {
                mainContentView.layer.maskedCorners = mainViewMaskedBounds
            } else {
                // Fallback on earlier versions
            }
        }
        
        if let cancelViewMaskedBounds = styleSettings[.cancelViewMaskedBounds] {
            if #available(iOS 11.0, *) {
                actionsSheetCancelButton?.layer.maskedCorners = cancelViewMaskedBounds
            } else {
                // Fallback on earlier versions
            }
        }
