    ...
    @objc func keyboardNotification(notification: NSNotification) {
        if let userInfo = notification.userInfo {
            ...
            if (endFrame?.origin.y)! >= globalPoint.y + alertView.frame.size.height {
                if let vc = (self as? CleanyAlertViewController), vc.preferredStyle == .actionSheet {
                    alertViewBottom?.constant = vc.bottomOffset
                } else {
                    alertViewCenterY.constant = 0
                }
            } else {
                if endFrame?.size.height != nil {
                    if (self as? CleanyAlertViewController)?.preferredStyle == .actionSheet {
                        alertViewBottom?.constant += endFrame?.height ?? 0
                    } else {
